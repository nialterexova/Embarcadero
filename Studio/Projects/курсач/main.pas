...
begin
if Application.MessageBox(Pchar('Вы точно хотите выйти?'),Pchar('Выход'),MB_YESNO)=mrYES then Close();
end;
...
begin
if Application.MessageBox(Pchar('Вы точно хотите удалить шаблон?'),Pchar(ExtractFileName(FileListBox1.FileName)),MB_YESNO)=mrYES then
...
begin
  ...
  if direct<>'template' then newForm.ShowModal else Application.MessageBox(Pchar('Выберете язык'),Pchar('Ошибка'),MB_OK);
  FileListBox1.Update;
end;
